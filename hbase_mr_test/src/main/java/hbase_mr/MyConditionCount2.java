...
public class MyConditionCount2 {
	...
	public static void main(String[] args){	
		...
		Job job;
		try {
			job = Job.getInstance(config, "MyConditionCount2");
			job.setJarByClass(MyConditionCount2.class);     // class that contains mapper
			
			Scan scan = new Scan();
			scan.setCaching(500);        // 1 is the default in Scan, which will be bad for MapReduce jobs
			scan.setCacheBlocks(false);  // don't set to true for MR jobs
			
			BinaryComparator comp = new BinaryComparator(Bytes.toBytes("20170223")); //
			SingleColumnValueFilter filter1 = new SingleColumnValueFilter(Bytes.toBytes("audiences"), Bytes.toBytes("05f8b534e090480899355a4b0b04ca58"), CompareOp.EQUAL, comp);
			SingleColumnValueFilter filter2 = new SingleColumnValueFilter(Bytes.toBytes("audiences"), Bytes.toBytes("2f39fca9cacb4da084d72e23954e8cb9"), CompareOp.NOT_EQUAL, comp);
			SingleColumnValueFilter filter3 = new SingleColumnValueFilter(Bytes.toBytes("audiences"), Bytes.toBytes("66109775faad42428ca9b73102533328"), CompareOp.EQUAL, comp);
			SingleColumnValueFilter filter4 = new SingleColumnValueFilter(Bytes.toBytes("audiences"), Bytes.toBytes("db87c6b7fd054e6e959c520279d5804f"), CompareOp.EQUAL, comp);
	...
	public static class MyMapper extends TableMapper<Text, IntWritable>  {
		...
		private final IntWritable ONE = new IntWritable(1);
	   	...
	   	public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException, InterruptedException {
	        	//String val = new String(value.getValue(Bytes.toBytes("audiences"), Bytes.toBytes("05f8b534e090480899355a4b0b04ca58")));
