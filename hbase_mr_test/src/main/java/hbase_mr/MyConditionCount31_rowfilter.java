...
public class MyConditionCount31_rowfilter {
	...
	public static void main(String[] args){	
		...
		Job job;
		try {
			job = Job.getInstance(config, "MyConditionCount31_rowfilter");
			job.setJarByClass(MyConditionCount31_rowfilter.class);     // class that contains mapper
			...
			ArrayList<Filter> allListForColumnFilters = new ArrayList<Filter>();
			Filter rpf = new PrefixFilter(Bytes.toBytes("10"));	
	        allListForColumnFilters.add(filter1);
	        allListForColumnFilters.add(filter2);
	        allListForColumnFilters.add(filter3);
	        allListForColumnFilters.add(filter4);
	        Filter columnFilterList = new FilterList(FilterList.Operator.MUST_PASS_ONE, allListForColumnFilters);
	        ArrayList<Filter> allFilters = new ArrayList<Filter>();
	        allFilters.add(rpf);
	        allFilters.add(columnFilterList);
	        Filter all_filter = new FilterList(FilterList.Operator.MUST_PASS_ALL, allFilters);
	        scan.setFilter(all_filter);
