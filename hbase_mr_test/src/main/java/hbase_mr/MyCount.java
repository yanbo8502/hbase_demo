...
public class MyCount {
	 ...
	 *      <property>
			...
			<property>
				...
				<value>sdf-spark-master1,sdf-spark-master2,sdf-resourcemanager1,sdf-resourcemanager2,sdf-logserver</value>
			</property>
	 * @param args
	 */
	public static void main(String[] args){	
		...
		Job job;
		try {
			job = Job.getInstance(config, "MyCount");
			job.setJarByClass(MyCount.class);     // class that contains mapper
			
			Scan scan = new Scan();
			scan.setCaching(500);        // 1 is the default in Scan, which will be bad for MapReduce jobs
			scan.setCacheBlocks(false);  // don't set to true for MR jobs
			// set other scan attrs...
			//scan.setStartRow(Bytes.toBytes("31883222"));
			//scan.setStopRow(Bytes.toBytes("41883222"));
